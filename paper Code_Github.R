# R Code for Analysis on Paper : Spatial cognitive ability is associated with movement behaviour of juvenile birds in a novel environment 
...
Spatial1 <- read.csv("SpatialLearning_NumErrors.csv")
...
m1 <- glmer(Straightness ~ ScaledDay * Spatial_Score + Sex + ScaledTestOrder + (1|ATLAS) + (1|N), data=Efficiency, family = Gamma(link="log"))
summary(m1)
drop1(m1, test="Chi")
...
m1 <- glmer(Straightness ~ ScaledDay + Spatial_Score + Sex + (1|ATLAS) + (1|N), data=Efficiency, family = Gamma(link="log"))
summary(m1)
drop1(m1, test="Chi")

m1 <- glmer(Straightness ~ ScaledDay + Sex + (1|ATLAS) + (1|N), data=Efficiency, family = Gamma(link="log"))
summary(m1)
drop1(m1, test="Chi")

m1 <- glmer(Straightness ~ ScaledDay + (1|ATLAS) + (1|N), data=Efficiency, family = Gamma(link="log"))
summary(m1)
drop1(m1, test="Chi")
